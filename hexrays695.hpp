...
#define NF_STROFF   0x20  ///< internal bit: used as stroff, valid iff isStroff()
//@}
...
int hexapi convert_to_user_call(const udcall_t &udc, mblock_t *blk, const insn_t &ins);
...
/// instrcuction all registered object will be visited by the following way:
///   if ( filter->match(ins) )
///     code = filter->apply(ins);
///   if ( code == MERR_OK )
...
struct microcode_filter_t
{
  ...
  /// \return success
  virtual bool match(const insn_t &ins) = 0;
  ...
  ///   else         - error
  virtual int apply(mblock_t *blk, const insn_t &ins) = 0;
};
...
public:
  ...
  inline udc_filter_t(const char *decl = NULL) { init(decl); }
  bool hexapi init(const char *decl);
  virtual int hexapi apply(mblock_t *blk, const insn_t &ins);
};
...
/// Get the vdui_t instance associated to the TForm
/// \param f pointer to window
/// \return a vdui_t *, or NULL

vdui_t *hexapi get_tform_vdui(TForm *f);
...
#define MERR_MAX_ERR   28
#define MERR_LOOP      (-29)   ///< internal code: redo last loop (never reported)
//@}
...
struct cnumber_t
{
  ...
  size_t hexapi print(char *buf, size_t bufsize, const tinfo_t &type, bool *nice_stroff=NULL) const;
...
//-V:citem_locator_t:730 not all members of a class are initialized inside the constructor
...
const int HEXRAYS_API_MAGIC2 = 0xBE3EBABE; // xxNNxxxx
...
//--------------------------------------------------------------------------
inline int convert_to_user_call(const udcall_t &udc, mblock_t *blk, const insn_t &ins)
{
  return (int)hexdsp(hx_convert_to_user_call, &udc, blk, &ins);
}
...
//--------------------------------------------------------------------------
inline int udc_filter_t::apply(mblock_t *blk, const insn_t &ins)
{
  return (int)hexdsp(hx_udc_filter_t_apply, this, blk, &ins);
}
...
//--------------------------------------------------------------------------
inline vdui_t *get_tform_vdui(TForm *f)
{
  return (vdui_t *)hexdsp(hx_get_tform_vdui, f);
}
...
//--------------------------------------------------------------------------
inline size_t cnumber_t::print(char *buf, size_t bufsize, const tinfo_t &type, bool *nice_stroff) const
{
  return (size_t)hexdsp(hx_cnumber_t_print, this, buf, bufsize, &type, nice_stroff);
}
